...
;(function($) {
  ...
  AlgoliaSearch = function(options) {
    ...
    var self = this
    var endpoint =
      'https://' + self.config.app_id + '-dsn.algolia.net/1/indexes/' + self.config.indexName
    ...
    self.buildMetadata = function(data) {
      ...
      self.dom.modal_metadata
        .children('.range')
        .html(self.nav.current + '-' + (self.nav.current + self.nav.currentCount - 1))
      if (self.nav.total > 0) {
